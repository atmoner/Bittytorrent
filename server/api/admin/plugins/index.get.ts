 ...
 */
export default defineEventHandler(async (event: H3Event): Promise<PluginState[]> => {
  const session = await getUserSession(event)
  if (!session?.user) {
    return sendError(event, createError({ statusCode: 401, statusMessage: 'Non autorisé.' })) as any
  }

  const db = await connectToDatabase()
  const user = await db.collection('users').findOne({ app_id: (session.user as any).id })
  if (!user || user.role !== 'admin') {
    return sendError(event, createError({ statusCode: 403, statusMessage: 'Accès interdit - Admin requis.' })) as any
  }

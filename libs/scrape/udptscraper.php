	...
	*/
	...
	class udptscraper extends tscraper{
		...
		public function scrape($url,$infohash){
			...
			if(count($infohash) > 74){ throw new ScraperException('Too many infohashes provided for:' . $infohash); }
			if(!preg_match('%udp://([^:/]*)(?::([0-9]*))?(?:/)?%si', $url, $m)){ throw new ScraperException('Invalid tracker url: '.$url); }
			$tracker = 'udp://' . $m[1];
			$port = isset($m[2]) ? $m[2] : 80;
			
			$transaction_id = mt_rand(0,65535);
			$fp = fsockopen($tracker, $port, $errno, $errstr);
			if(!$fp){ throw new ScraperException('Could not open UDP connection: ' . $errno . ' - ' . $errstr .' for tracker:' . $tracker,0,true); }
			...
			//Connection response
			$ret = fread($fp, 16);
			if(strlen($ret) < 1){ throw new ScraperException('No connection response for tracker: ' . $tracker,0,true); }
			...
			if(strlen($ret) < 1){ throw new ScraperException('No scrape response for tracker: ' . $tracker,0,true); }
			if(strlen($ret) < 8){ throw new ScraperException('Too short scrape response for tracker: ' . $tracker); }
			$retd = unpack("Naction/Ntransid",$ret);
			...
			if(strlen($ret) < $readlength){ throw new ScraperException('Too short scrape response for tracker: ' . $tracker); }
			$torrents = array();
			$index = 8;
			foreach($infohash as $hash){
				$retd = unpack("Nseeders/Ncompleted/Nleechers",substr($ret,$index,12));
				$retd['infohash'] = $hash;
				$retd['tracker'] = $tracker;
				$torrents[$hash] = $retd;
				$index = $index + 12;
			}
			//var_dump($torrents);

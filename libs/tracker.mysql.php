...
$_SERVER['tracker'] = array(
	...
	// database options
	'db_host'           => $dbhost,   /* ip or hostname to mysql server */
	'db_user'           => $dbuser,        /* username used to connect to mysql */
	'db_pass'           => $dbpass,            /* password used to connect to mysql */
	'db_name'           => $dbname, /* name of the PeerTracker database */
	...
	public static function updateUser()
	{
		...
		if ($_SERVER['tracker']['open_tracker'] === 'false') {
			if (!isset($_GET['pid']) || strlen($_GET['pid']) != 32) 
				...
				$user = $db->get_row("SELECT id FROM users WHERE private_id = '".$db->escape($_GET['pid'])."' LIMIT 1"); 
				if ($user) {
					$db->query("UPDATE users SET   
								upload=IFNULL(upload,0)+ ".$db->escape($_GET['uploaded']).",
								download=IFNULL(download,0)+ ".$db->escape($_GET['downloaded'])."

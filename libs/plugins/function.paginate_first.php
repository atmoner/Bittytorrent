...
function smarty_function_paginate_first($params, &$smarty) {
	...
	if ($cat_id!='NULL') {
		if (!empty($_GET["sortedBy"]) || !empty($_GET["axis"])) {
			$final_url = $startUp->makeUrl(array('page'=>$startUp->paginatePage,'gost'=>'cat','catid'=>$cat_id,'next'=>'1','sortedBy'=>$_GET["sortedBy"],'axis'=>$_GET["axis"])); 
		} else
		...
		if (!empty($_GET["sortedBy"]) || !empty($_GET["axis"])) {
			$final_url = $startUp->makeUrl(array('page'=>$startUp->paginatePage,'next'=>'1','sortedBy'=>$_GET["sortedBy"],'axis'=>$_GET["axis"])); 
		} else
		$final_url = $startUp->makeUrl(array('page'=>$startUp->paginatePage,'next'=>'1'));
	}

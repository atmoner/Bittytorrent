...
define('UPDATE_DIR_INSTALL', '/var/www/');
...
class AutoUpdate {
	 ...
	 * Github url
	 */
	public $gitUrl = 'https://github.com/atmoner/Bittytorrent/archive/master.zip';
	...
	public function install($updateFile) {
		...
		while ($file = zip_read($zip)) {				
			...
			$filename = str_replace("Bittytorrent-master","",$filename);			
		...
		zip_close($zip);
		$this->recursiveChmod($foldername,0755,0755);
	...
	public function update() {
		...
		if ($this->latestVersion > $this->currentVersion) {
			...
			$gitUrl = 'https://github.com/atmoner/Bittytorrent/archive/master.zip';
			
			//Download update
			if (!is_file($gitUrl)) {
				if (!$this->downloadUpdate($this->gitUrl, $updateFile)) {
